<script lang="ts">
    ...
    //import {FieldRobot} from "@frc-web-components/fwc/components/field"
    ...
    let robotPose = NT.NTDoubleArray([0,0,0], "/Robot/m_driverField/Robot")
    let targetPose = NT.NTDoubleArray([0,0,0], "/Robot/m_driverField/target")
...
<div
    ...
    grid-template-rows:repeat(8, calc(calc(100vh - 200px) / 4));
">
...
<GridItem x={8} y={0} width={2} height={5}>
    <frc-field rotation={$isBlue ? 90 : 270} rotation-unit="deg" style="width:100%;height:100%;" game="Crescendo">
        <frc-field-robot
        pose={($robotPose)} 
